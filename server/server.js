...
// ✅ Simple health check route (so / pe 404 na aaye)
app.get("/", (req, res) => {
  res.send("✅ AI Autonomous System backend is running.");
});
...
app.post("/api/hf", async (req, res) => {
  try {
    const { inputs } = req.body; // frontend se string aati hai
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a helpful AI assistant. Respond clearly and do not include any hidden reasoning markup like <think>.",
          },
          {
            role: "user",
            content: inputs,
...
// ---------------------------------------------------------------------
// 3) START SERVER (Render-friendly PORT)
// ---------------------------------------------------------------------
const PORT = process.env.PORT || 3001;
app.listen(PORT, () => {
  console.log(`Backend running on port ${PORT}`);
